...
resource "azurerm_network_interface" "interfaceredevm-kafka" {
  ...
  ip_configuration {
    ...
    public_ip_address_id = local.tags["change_ip"] == 0 ? azurerm_public_ip.ippublico-kafka.id : null
  }
...
resource "azurerm_virtual_machine" "vm-linux-kafka" {
  ...
  provisioner "remote-exec" {
    connection {
      type = "ssh"
      host = azurerm_public_ip.ippublico-kafka.ip_address
